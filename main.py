...
def get_filename_from_content_disposition(response: Response) -> str:
    content_disposition = response.headers.get('Content-Disposition')
    file_name = unquote_plus(content_disposition.split(";")[1].split('"')[1]) # FRAGILE
    return file_name

def download_file(url: str, directory_path: str) -> str:
    with get(url, stream=True) as resp:
        resp.raise_for_status()
        file_name = get_filename_from_content_disposition(resp)
...
def download_bd_files(set_url: str, directory_path: str, limit: int|None = None) -> list[str]:
...
if __name__ == "__main__":
    ...
    files = download_bd_files(report_url, out_dir, limit=3)
